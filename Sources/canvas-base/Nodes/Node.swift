...
typealias NodeKey = Key<XNode>

// temp - for optimizing memory layout / size
final class XNode {
    
    var key: NodeKey
    var graph: InternalDirectSnapshot
    
    init(key: NodeKey = NodeKey(), graph: DAG) {
        self.key = key
        self.graph = graph as! InternalDirectSnapshot
    }
...
class Node: Hashable, CustomDebugStringConvertible {
    ...
    var type: DNodeType { graph.type(for: key)! }
    
    var contentHash: Int { fatalError() }
    
    init(_ key: Key = .init(), graph: DAG) {
        self.key = key
        self.graph = graph
    }
    ...
    @discardableResult
    func add(to graph: MutableDAG, useFreshKeys: Bool) -> NodeKey {
    ...
    func add(diffTo graph: MutableDAG, parent: DAG) {
        let source = graph
    ...
    final func equal(to other: Node, ignoringKey: Bool = false) -> Bool {
    ...
    final func equalKey(to other: Node) -> Bool {
        return key == other.key
    }
    
    final func equalInputs(to other: Node, ignoringKey: Bool) -> Bool {
        let ai = inputs
        let bi = other.inputs
    ...
    func equalPayload(to other: Node) -> Bool {
        fatalError()
    }
    ...
    func replace(_ keyToReplace: NodeKey, with replacement: Node) {
        fatalError()
    }
    
    func foreach(_ block: (Node)->()) {
        for input in inputs {
            input.foreach(block)
        }
    ...
    func all(where predicate: (Node)->Bool) -> [Node] {
    ...
    func all<U: Node>(as type: U.Type) -> [U] {
