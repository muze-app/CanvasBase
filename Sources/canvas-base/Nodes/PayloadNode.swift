...
class PayloadNode<PayloadType: NodePayload>: Node {
    ...
    init(_ key: NodeKey = NodeKey(), graph: DAG, payload: PayloadType? = nil, nodeType: DNodeType? = nil) {
    ...
    @discardableResult
    override func add(to graph: MutableDAG, useFreshKeys: Bool) -> NodeKey {
        let newKey = super.add(to: graph, useFreshKeys: useFreshKeys)
        graph.setPayload(payload, for: newKey)
        return newKey
    }
    
    override func add(diffTo graph: MutableDAG, parent: DAG) {
    ...
    final override var contentHash: Int {
