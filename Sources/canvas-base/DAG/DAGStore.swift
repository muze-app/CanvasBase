...
//            commits[$0]!
    ...
    init(delegate: DAGStoreDelegate? = nil) {
        self.delegate = delegate
        
        let graph = InternalDirectSnapshot(store: self, level: 0)
        commit(graph)
    }
    ...
    func commit(for key: CommitKey) -> InternalDirectSnapshot? {
        var commit: InternalDirectSnapshot?
        sync { commit = commits[key] }
        return commit
    }
    
    @discardableResult
    func commit(_ snapshot: InternalDirectSnapshot, process: Bool = true) -> CommitKey {
...
//            }
    ...
    func retain(commitFor key: CommitKey, mode: DAGSnapshot.Mode) {
        sync {
            ...
            guard commitTimes[key].exists else {
                fatalError()
            }
...
protocol DAGStoreDelegate: class {
    ...
    func considerPurging(_ nodes: [(DNode, Subgraph)])
